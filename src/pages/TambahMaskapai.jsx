...
import Button from "../components/Button";
import { createMitra } from "../redux/actions/mitraAction"; // Assuming mitraAction.js is in "../actions/"

const TambahMaskapai = ({ isSidebarOpen }) => {
  const dispatch = useDispatch();
  const { loadingCreate, errorCreate, createdMitra } = useSelector((state) => state.mitra); // Get state from mitraReducer
  ...
  // Handle Submit button click - dispatch createMitra action
  const handleSubmit = () => {
    // Check if fields are empty
    if (!name || !description) {
      alert("Please fill in both fields.");
    ...
    // Dispatch the createMitra action with the new airline data
    dispatch(createMitra({ name, description }));
  };

  // Effect to handle success or error after creating mitra
  useEffect(() => {
    if (createdMitra) {
      alert("Airline added successfully!");
      handleReset();
    ...
    if (errorCreate) {
      alert(`Error adding airline: ${errorCreate}`);
    }
  }, [createdMitra, errorCreate]);
        ...
        <div className="bg-white m-4 py-4 rounded-lg shadow-md">
          <div className="flex-col px-4 items-center">
            ...
            <div className="flex flex-row justify-center gap-6 text-white font-bold mt-12 mb-10">
              <Button text="Reset" bgColor="bg-yellow1" onClick={handleReset} />
              <Button
                text={loadingCreate ? "Submitting..." : "Submit"} // Show loading state
                bgColor="bg-blue1"
                onClick={handleSubmit}
                disabled={loadingCreate} // Disable button while submitting
              />
            </div>
          </div>
        </div>
