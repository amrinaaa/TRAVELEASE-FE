...
const TambahRuangan = ({ isSidebarOpen }) => {
  ...
  // --- Component State for Success Popup ---
  const [showSuccessPopup, setShowSuccessPopup] = useState(false);
  const [successMessage, setSuccessMessage] = useState('');
  const [onCloseSuccessPopupCallback, setOnCloseSuccessPopupCallback] = useState(null);
  ...
  useEffect(() => {
    if (createdRoomData) {
      ...
      setOnCloseSuccessPopupCallback(() => () => {
        dispatch(resetCreateRoomStatus());
        navigate(`/manajemen-ruangan/${hotelId}`);
      });
    }
    if (errorCreateRoom) {
      // You might want to implement an error popup as well
      alert(`Error creating room: ${errorCreateRoom}`);
      dispatch(resetCreateRoomStatus());
    }
  }, [createdRoomData, errorCreateRoom, dispatch, navigate, hotelId]);
  ...
  useEffect(() => {
    if (createdRoomTypeData) {
      ...
      setOnCloseSuccessPopupCallback(() => () => {
        dispatch(resetCreateRoomTypeStatus());
        // Optionally, select the newly created room type if needed:
        // setSelectedRoomTypeId(createdRoomTypeData.id);
      });
    }
    if (errorCreateRoomType) {
  ...
  useEffect(() => {
    if (createdFacilityData) {
      setSuccessMessage(`Facility "${createdFacilityData.facilityName}" created successfully as a global facility! Now add it to the room type.`);
      setShowSuccessPopup(true);
      // Add the facility (that was just globally created) to the current newType's facility list
      setNewType(prevType => ({
        ...prevType,
  ...
  }, [createdFacilityData, errorCreateFacility, dispatch, newFacility.amount]);
  ...
  const handleSaveNewType = () => {
    ...
    const roomTypePayload = {
      ...
      capacity: parseInt(newType.capacity, 10),
      price: priceValue,
      facilities: newType.facilities,
    };
    dispatch(createRoomType(roomTypePayload));
  };

  // --- Handlers for "Add Facility" Modal ---
  const handleFacilityModalInputChange = (e) => {
    const { name, value } = e.target;
    if (name === "facilityName") {
        setNewFacility(prev => ({ ...prev, name: value }));
  ...
  const handleSaveNewFacilityAndAddToType = () => {
    if (!newFacility.name.trim()) {
      alert("Facility name cannot be empty!");
      return;
    }
  ...
  // --- Handler for closing success popup ---
  const closeSuccessPopup = () => {
    setShowSuccessPopup(false);
    setSuccessMessage('');
    if (typeof onCloseSuccessPopupCallback === 'function') {
      onCloseSuccessPopupCallback(); // Execute the stored callback
    }
    setOnCloseSuccessPopupCallback(null); // Reset callback
  };
  ...
  return (
    <div className="flex transition-all duration-300">
      ...
      {showTypeModal && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-purple-50 p-6 rounded-lg w-96 max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Add New Room Type</h2>
              <button type="button" onClick={() => { setShowTypeModal(false); setNewType({ name: '', facilities: [], capacity: 0, price: '' }); }} className="text-gray-500" >
                <i className="fas fa-times"></i>
      ...
      {/* Success Popup Modal */}
      {showSuccessPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-[60] p-4"> {/* Increased z-index */}
          <div className="bg-white p-6 rounded-lg shadow-xl text-center transform transition-all sm:max-w-xs sm:w-full">
            <div className="mb-4">
              {/* Ensure Remix Icon is available or replace with another icon */}
