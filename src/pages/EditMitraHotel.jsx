...
import { getHotelDetail, editHotelPartner } from "../redux/actions/adminHotelActions"; // Pastikan nama path sesuai dengan folder Anda
import { resetHotelState } from "../redux/reducers/adminHotelReducer";

const EditMitraHotel = () => {
  const { mitraName } = useParams(); // Mendapatkan nama mitra dari URL
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { hotelDetail, loadingFetch, errorFetch, successEdit, errorEdit, loadingEdit } = useSelector(state => state.adminHotel);
  ...
  useEffect(() => {
    if (mitraName) {
      dispatch(getHotelDetail(mitraName)); // Ambil detail hotel berdasarkan mitraName
    }
  }, [mitraName, dispatch]);
  ...
  useEffect(() => {
    if (successEdit) {
      ...
      navigate("/manajemen-mitra-hotel"); // Redirect to the hotel management page
    }
    if (errorEdit) {
  ...
  if (errorFetch) {
    return <p>Error: {errorFetch}</p>;
  }

  return (
    <div>
      <h2>Edit Mitra Hotel</h2>
