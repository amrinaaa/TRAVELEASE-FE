...
const TableSearchHotel = ({ onHotelFilter, onFlightSearch, activeTab: propActiveTab, onTabChange, initialSearchData }) => {
  ...
  const handleHotelSubmit = () => {
    ...
    // Panggil fungsi filter dari parent component
    if (onHotelFilter) {
  ...
  const handleFlightSubmit = () => {
    ...
    // Jika onFlightSearch ada (untuk halaman yang sama), gunakan itu
    if (onFlightSearch) {
      onFlightSearch(flightForm);
    } else {
      // Jika tidak ada onFlightSearch, navigasi ke halaman pesawat dengan data pencarian
      navigate('/pesawat', {
        state: {
...
export default TableSearchHotel