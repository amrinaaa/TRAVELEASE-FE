...
const NavbarMitraHotel = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  const location = useLocation(); // // Tetap dipertahankan jika diperlukan untuk logika lain
  ...
  const defaultProfilePic = "https://placehold.co/40/7B68EE/FFFFFF&text=H"; // Placeholder dengan inisial Hotel
  ...
  const handleLogout = () => {
    ...
    window.location.href = "/login"; // Arahkan ke login mitra umum atau spesifik hotel
  };
            ...
            <div className='absolute bg-white rounded-lg shadow-lg w-52 top-full right-3 border z-20 animate-fadeIn'>
              ...
              <ul className='text-left py-1 text-sm text-gray-700'> {/* */}
                <li className='px-3 py-2 hover:bg-gray-100'> {/* */}
                  <Link
                    to="/pengaturan-akun-mitra-hotel" // Path diperbarui
                ...
                <li className='px-3 py-2 hover:bg-gray-100 border-t border-gray-100'> {/* */}
                  <button className="flex items-center w-full text-left"> {/* */}
                    ...
                    {/* Saldo: Rp. 90,000 (Jika ada dari API, bisa diganti) */} {/* */}
                    Info Keuangan (Contoh)
                  </button>
                </li>
