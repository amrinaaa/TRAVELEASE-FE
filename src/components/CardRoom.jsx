...
import dataRuangan from '../utils/dataRuangan.json';  // Impor data dummy

const CardRoom = () => {
  return (
    <div>
      {dataRuangan.map((room) => (
        <div className="flex md:flex-row flex-col md:p-0 p-6 md:mb-4" key={room.id}>
          <div className="flex-col md:grid md:grid-cols-3  w-full bg-ungu10 md:rounded-l-3xl md:rounded-tr-none rounded-tr-3xl rounded-tl-3xl shadow-lg p-8 flex items-center text-justify justify-between gap-4">
            ...
            <div className="flex flex-col col-span-2 text-justify gap-2">
              <div>
                <span className="font-bold text-black text-2xl">{room.name}</span>
              </div>
              <div>
              ...
              <div className="flex flex-col mt-1 mb-1">
                <span className="font-bold text-black">Capacity : {room.capacity}</span>
              </div>
              <div className="flex flex-col mt-1 mb-1">
                <span className="font-bold text-black">Status : {room.status}</span>
              </div>
              <div className="flex flex-col mt-2 mb-2">
          ...
          <div>
            <Link to={`/detail-ruangan/${room.id}`}>
