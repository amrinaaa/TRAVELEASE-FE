...
const initialState = {
  ...
  successEdit: false,
  errorEdit: null,
  editedHotel: null, // Store the edited pesawat data
    ...
    createHotelRequest: (state) => {
     ...
     deleteHotelRequest: (state) => {
      ...
      getHotelDetailSuccess: (state, action) => {
        state.loadingFetch = false;
        state.hotelDetail = action.payload; // Simpan pesawat detail yang didapat
        state.errorFetch = null;
      },
...
} = adminPesawatSlice.actions;

export default adminPesawatSlice.reducer;
